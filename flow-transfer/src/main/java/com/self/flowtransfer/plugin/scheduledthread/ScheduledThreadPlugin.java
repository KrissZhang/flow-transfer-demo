package com.self.flowtransfer.plugin.scheduledthread;
...
public class ScheduledThreadPlugin implements IPlugin {
    ...
    private Runnable createRunnableJob(String jobName, String jobClassName) {
        ...
        Object temp = null;
        try {
            String[] jobClass = jobClassName.split("@");
            Class[] classes = new Class[jobClass.length - 1];
            Arrays.fill(classes, String.class);
            temp = Class.forName(jobClass[0]).getConstructor(classes)
                    .newInstance(Arrays.copyOfRange(jobClass, 1, jobClass.length));
        } catch (Exception e) {
