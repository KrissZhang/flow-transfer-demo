package com.self.flowtransfer.log4j;
 ...
 * 日志配置类
 */
public class FlowTransferRollingFileAppender extends RollingFileAppender {
    private LocalDate localDate = null;
    private String parternFileName = null;
    private long nextRollover = 0;

    public FlowTransferRollingFileAppender() {
        super();
    }

    public FlowTransferRollingFileAppender(Layout layout, String filename, boolean append)
            throws IOException {
    ...
    public FlowTransferRollingFileAppender(Layout layout, String filename)
            throws IOException {
    ...
    protected void subAppend(LoggingEvent event) {
        ...
        if(fileName != null && qw != null) {
            long size = ((CountingQuietWriter) qw).getCount();
            if ((size >= maxFileSize && size >= nextRollover) || !localDate.equals(LocalDate.now())) {
                rollOver = true;
            }
        }
